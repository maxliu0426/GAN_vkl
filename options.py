...
def option_initialize():
    ...
    flag.DEFINE_integer('iters',100000, 'iterations to train')
    flag.DEFINE_float('lr', 0.0002, 'learning rate for adam')
    ...
    flag.DEFINE_integer('lamda', 1, 'hyperparameter for the loss function')
    flag.DEFINE_integer('input_size', 32, 'input image size')
    flag.DEFINE_integer('output_size', 32, 'output image size')
    flag.DEFINE_integer('batch_size', 64, 'the size of image batch')
    flag.DEFINE_string('dataset', 'cifar', 'the name of dataset[celebA ,cifar]')
    ...
    flag.DEFINE_boolean('crop', False, 'True for crop, False for not')
